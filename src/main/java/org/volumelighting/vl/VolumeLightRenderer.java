...
/**
 * BasicShadowRenderer uses standard shadow mapping with one map it's useful to
 * render shadows in a small scene, but edges might look a bit jagged.
 *
 ...
 * @author Toucher
 */
public class VolumeLightRenderer extends AbstractShadowRenderer {
     ...
     */
    public VolumeLightRenderer(AssetManager manager, int size) {
...
//        }
        ...
        // We prevent computing the frustum points and splits with zeroed or negative
        // near clip value
//        float frustumNear = Math.max(viewCam.getFrustumNear(), 0.001f);
//        ShadowUtil.updateFrustumPoints(viewCam, frustumNear, zFar, 1.0f, points);
        // shadowCam.setDirection(direction);
        // shadowCam.setFrustumPerspective(spot.getSpotOuterAngle() *
        // FastMath.RAD_TO_DEG * 2.0f, 1, 1f, spot.getSpotRange());
...
//        }
    ...
    private Matrix4f lightViewProjectionsMatrix = new Matrix4f();
    private GeometryList shadowMapOccluders = new GeometryList(new OpaqueComparator());
    ...
    protected GeometryList getOccludersToRender(int shadowMapIndex, GeometryList sceneOccluders,
            GeometryList sceneReceivers, GeometryList shadowMapOccluders) {
        getGeometriesInCamFrustum(sceneOccluders, shadowCam, shadowMapOccluders);
        return shadowMapOccluders;
    }

    protected void getGeometriesInCamFrustum(GeometryList inputGeometryList, Camera camera, GeometryList outputGeometryList) {
    ...
    /**
     * sets the light direction to use to computes shadows
     *
     * @param direction
     */
